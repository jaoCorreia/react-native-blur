...
2. **Method selection** — radius > 5 (with `BlurMethod::Auto`) → `boxBlur3Pass()` (`boxblur.cpp`, O(N), radius-independent, **scalar** — no SIMD); otherwise separable Gaussian (horizontal + vertical 1D passes). Note: because box runs at radius >5 and Gaussian's SIMD path is fast, box is actually *slower* than Gaussian for mid radii (~6–16 at 1024²); the ">5" threshold is not perf-optimal.
